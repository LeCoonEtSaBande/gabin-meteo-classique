...
test("spots_specifications ne contient que Ferme de Sauze", () => {
  ...
  assert.equal(rows[0].link_windguru, undefined);
  assert.equal(rows[0].link_webcam, undefined);
  assert.equal(rows[0].link_anemometer, undefined);
  assert.equal(rows[0].Latitude_mise_a_leau, undefined);
  assert.equal(rows[0].Longitude_mise_a_leau, undefined);
